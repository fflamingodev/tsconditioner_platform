...
// --------- OldRegularize (tolerance ignored -> set 0) ---------
...
func TestRegularize_Average(t *testing.T) {
	...
	got := ts.OldRegularize(30, "s", "avg", 0) // tolérance ignorée
	base := mustTime(2025, 11, 10, 10, 0, 0)
	want := TimeSeries{}
	want.AddDataUnit(
...
func TestRegularize_Maximum(t *testing.T) {
	ts := buildSimpleSeries()
	got := ts.OldRegularize(30, "s", "max", 0)
	base := mustTime(2025, 11, 10, 10, 0, 0)
	want := TimeSeries{}
	want.AddDataUnit(
...
func TestRegularize_Minimum(t *testing.T) {
	ts := buildSimpleSeries()
	got := ts.OldRegularize(30, "s", "min", 0)
	base := mustTime(2025, 11, 10, 10, 0, 0)
	want := TimeSeries{}
	want.AddDataUnit(
...
func TestRegularize_Last(t *testing.T) {
	ts := buildSimpleSeries()
	got := ts.OldRegularize(30, "s", "last", 0)
	base := mustTime(2025, 11, 10, 10, 0, 0)
	want := TimeSeries{}
	want.AddDataUnit(
...
func TestRegularize_Sum(t *testing.T) {
	ts := buildSimpleSeries()
	got := ts.OldRegularize(30, "s", "sum", 0)
	base := mustTime(2025, 11, 10, 10, 0, 0)
	want := TimeSeries{}
	want.AddDataUnit(
...
func TestRegularize_PerAlias(t *testing.T) {
	...
	ref := ts.OldRegularize(2, "s", "last", 0)
	alts := []string{"sec", "Seconds", "s"}
	for _, per := range alts {
		got := ts.OldRegularize(2, per, "last", 0)
		requireSeriesEq(t, got, ref, 0)
	}
}

// --------- OldRegularize: insertion de NaN quand il manque des fenêtres ---------
...
func TestRegularize_InsertNaNs_WhenGaps_Average(t *testing.T) {
	...
	got := ts.OldRegularize(30, "s", "avg", 0) // tolérance ignorée -> 0
