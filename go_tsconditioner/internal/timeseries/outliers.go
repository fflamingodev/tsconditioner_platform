...
func (tsin *TimeSeries) RemoveOutbounds(
	min, max *float64,
	comment string,
...
func (tsin *TimeSeries) LowerPercCleaning(perc float64) (TimeSeries, TimeSeries) {
	tsin.SortMeasAsc()
	dt := tsin.MeasToArr()
	min, _ := Percentile(dt, perc)
	return tsin.RemoveOutbounds(&min, nil, "Percentile ("+fmt.Sprintf("%.2f", min)+") - "+time.Now().Round(0).String())
}
func (tsin *TimeSeries) UpperPercCleaning(perc float64) (TimeSeries, TimeSeries) {
	tsin.SortMeasAsc()
	dt := tsin.MeasToArr()
	max, _ := Percentile(dt, (100 - perc))
	return tsin.RemoveOutbounds(nil, &max, "Percentile ("+","+fmt.Sprintf("%.2f", max)+") - "+time.Now().Round(0).String())
}
